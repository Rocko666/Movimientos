...
vApp="ReadExcel COMBO_INICIAL.xlsx"
...
print(lne_dvs())
vStp01='Paso [1]: Se lee el archivo excel COMBO_INICIAL'
try:
    ts_step = datetime.now()
    dfExcel = pd.read_excel(vPathExcel, error_bad_lines=False,dtype=str)
    te_step = datetime.now()
...
try:
    ...
    for vColumn in vColumns:
        if (re.match( vRegExpUnnamed,vColumn.lower())):
            ...
            df1 = df1.withColumnRenamed(vColumn,getColumnName(vColumn))
    ...
    if df2.rdd.isEmpty():
        ...
        df3 = df3.withColumn('fecha_proceso',date_format(df3.fecha_proceso,'yyyyMMdd'))
        df3 = df3.withColumn('fecha_alta',date_format(df3.fecha_alta,'dd/MM/yyyy'))
...
print(etq_info(msg_d_duracion_ejecucion('otc_t_combo_inicial.py',vle_duracion(timestart,timeend))))
print(lne_dvs())
