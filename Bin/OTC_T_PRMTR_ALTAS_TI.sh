#!/bin/bash
############################################################################
#   Script de carga de archivo de perimetros para los altas y transfer in  #
...
/usr/hdp/current/spark2-client/bin/spark-submit --master yarn --executor-memory 5G --num-executors 4 --executor-cores 6 --driver-memory 4G ${VAL_RUTA}/Python/OTC_T_PRMTR_ALTAS_TI.py --rutain="${VAL_RUTA}/Input/$VAL_FTP_NOM_ARCHIVO" --tablaout=$VAL_DIR_HDFS_CAT --tipo=overwrite --part $VAL_FECHA_EJE 1>> $VAL_LOG 2>> $VAL_LOG
...
### sh -x /home/nae108834/Perimetro_movimientos/Bin/OTC_T_PRMTR_ALTAS_TI.sh 20220930