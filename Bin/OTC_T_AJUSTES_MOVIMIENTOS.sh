...
#!/bin/bash
############################################################################
#   Script de carga de archivo AJUSTES_MOVIMIENTOS.xlsx 		   #
...
ENTIDAD=OTC_T_AJUSTES_MOVIMIENTOS
...
#INICIO DEL PROCESO
echo "==== Inicia ejecucion AJUSTES_MOVIMIENTOS.xlsx===="`date '+%Y%m%d%H%M%S'` 2>&1 &>> $VAL_LOG
#PASO 1: REALIZA LA TRANSFERENCIA DE LOS ARCHIVOS CSV DESDE EL SERVIDOR FTP A RUTA LOCAL EN BIGDATA
if [ "$ETAPA" = "1" ]; then
echo "==== Realiza la transferencia de AJUSTES_MOVIMIENTOS.xlsx desde el servidor FTP a BigData ===="`date '+%Y%m%d%H%M%S'` 2>&1 &>> $VAL_LOG
...
echo "==== Finaliza la transferencia de AJUSTES_MOVIMIENTOS.xlsx===="`date '+%Y%m%d%H%M%S'` 2>&1 &>> $VAL_LOG
...
$VAL_RUTA/Python/otc_t_ajustes_movimientos.py \
...
### sh -x /home/nae108834/Movimientos/Bin/OTC_T_AJUSTES_MOVIMIENTOS.sh