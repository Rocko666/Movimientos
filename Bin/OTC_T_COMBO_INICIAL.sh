...
#!/bin/bash
############################################################################
#   Script de carga de archivo COMBO_INICIAL.xlsx 		   #
...
#INICIO DEL PROCESO
echo "==== Inicia ejecucion COMBO_INICIAL.xlsx===="`date '+%Y%m%d%H%M%S'` 2>&1 &>> $VAL_LOG
#PASO 1: REALIZA LA TRANSFERENCIA DE LOS ARCHIVOS CSV DESDE EL SERVIDOR FTP A RUTA LOCAL EN BIGDATA
if [ "$ETAPA" = "1" ]; then
echo "==== Realiza la transferencia de COMBO_INICIAL.xlsx desde el servidor FTP a BigData ===="`date '+%Y%m%d%H%M%S'` 2>&1 &>> $VAL_LOG
...
echo "==== Finaliza la transferencia de COMBO_INICIAL.xlsx===="`date '+%Y%m%d%H%M%S'` 2>&1 &>> $VAL_LOG
...
$VAL_RUTA/Python/otc_t_combo_inicial.py \
...
### sh -x /home/nae108834/Movimientos/Bin/OTC_T_COMBO_INICIAL.sh